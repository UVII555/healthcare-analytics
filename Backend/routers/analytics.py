...
from analytics.engine.kpi_engine import (
    compute_alos, compute_rar, compute_awt, compute_drg_mix,
    compute_opd_load, compute_bor, compute_cpv, compute_revenue
)
...
def get_kpis(hospital_id: int = 1, db: Session = Depends(get_db)):
    return {
        ...
        "bor_percent": compute_bor(db, hospital_id),
        "cpv_rupees": compute_cpv(db, hospital_id),
        "revenue_mtd": compute_revenue(db, hospital_id),
    }
